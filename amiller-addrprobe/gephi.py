...
def adjust_sizes(nodes, attribute="in_degree", scale=None):
    ...
    for node in nodes:
        ...
        node_data.setSize(0.1 + scale(value)) # avoid problem with 0 sizes
        # print "changed %s size from %s to %s" % (node_data.getLabel(), value, (1 + scale(value)))

def layout(graph_file='./test.gexf', out_gexf='./new_graph.gexf', layout=True, save_pdf=True, save_gephi=False):
    print "loading graph"
    # tips:
    ...
    if layout:
        ...
        force_atlas = ForceAtlas2(None)
        #fa = lookup('layout.plugin.forceAtlas2.ForceAtlas2')
        ...
        #print force_atlas, dir(force_atlas)
        #force_atlas.adjustSizes = True
        #force_atlas.attractionStrength = 8
        #force_atlas.setRepulsionStrength(2000)
        force_atlas.setGravity(force_atlas.getGravity()/4)
    ...
    preview.properties.putValue('node.border.width', 1)
    preview.properties.putValue('node.label.show', False)
    PreviewController.refreshPreview() # necessary?

    #from org.gephi.preview.api import PreviewProperty
    #gm.getModel().getProperties().putValue(PreviewProperty.SHOW_NODE_LABELS, Boolean.TRUE);
    ...
    if save_pdf:
        ...
        part, ext = os.path.splitext(out_gexf)
        out_pdf = part + '.png'
        pdf = ExportController.getExporter("png")
        ExportController.exportFile(java.io.File(out_pdf), pdf)
