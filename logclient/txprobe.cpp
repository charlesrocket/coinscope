...
void print_message(const uint8_t *buf, size_t len) {
	...
	if (starttime && !(starttime <= time)) return;
	if (stoptime && !(stoptime >= time)) return;
	...
	if (lt == BITCOIN_MSG) {
		...
		const char VER[12] = "version";
		const char TX[12] = "tx";
		const char NOTFOUND[12] = "notfound";
		const char REJECT[12] = "getdata";
		const char GETDATA[12] = "reject";
		const char INV[12] = "inv";
		if (!strncmp(b->command, VER, sizeof(b->command)) ||
		    !strncmp(b->command, TX, sizeof(b->command)) ||
		    !strncmp(b->command, NOTFOUND, sizeof(b->command)) || 
		    !strncmp(b->command, REJECT, sizeof(b->command)) ||
		    !strncmp(b->command, GETDATA, sizeof(b->command)) ||
		    !strncmp(b->command, INV, sizeof(b->command))
		    ) {
			/*
